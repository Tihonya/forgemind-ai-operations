...
- Application images are built ONCE for S (serialized, one at a time). The Model C candidate artifact identity is: repository SHA + build-time input values + verified application image IDs. The Git SHA alone does not identify a rebuild with a changed build arg — the SHA may stay the same while the image IDs change.
- The application image references are deterministic: `docker-compose.prod.yml` declares `name: forgemind` and backend / worker / frontend use `build:` with no explicit `image:`, so Docker Compose v2 resolves them as `forgemind-backend` / `forgemind-worker` / `forgemind-frontend`.
- WP-P7-07 verifies staging against exactly S and records/validates the build-time input values (currently: `VITE_API_BASE_URL`, non-secret; Release 1 expected/default `/api/v1`) and the image identities (backend image ID, worker image ID, frontend image ID; the full Compose-resolved service image inventory as additional evidence where cheap and unambiguous).
...
- Production checks out the same SHA S, verifies S before starting, verifies the intended build-time input values equal the staging evidence, verifies required image IDs equal the staging evidence, and starts with no-build / no-pull. Production MUST NOT rebuild application images and MUST NOT pull replacement images between staging verification and promotion. Production startup fails closed if required verified images are absent, their identities differ, or a build-time input value differs.
- Production runtime configuration / FQDN / secrets may differ from staging ONLY where they are genuinely runtime inputs (the final FQDN `CADDY_DOMAIN`, `CADDY_EMAIL`, database/Redis credentials, and other container environment values that are not compiled into an image). BUILD-TIME inputs MUST NOT differ: the explicit build-time input is `VITE_API_BASE_URL` — compiled into the frontend image (frontend Dockerfile `ARG`/`ENV` → Vite statically substitutes `import.meta.env.VITE_API_BASE_URL` during `npm run build`; nginx performs no runtime substitution). If `VITE_API_BASE_URL` (or any future build-time input) changes after staging verification, the verified frontend image is no longer the artifact intended for production and production promotion MUST STOP — do NOT rebuild during promotion; the rebuilt application image set is a NEW candidate artifact set that must repeat staging deployment/verification before production promotion.
...
- The deployment contract now defines WP-P7-06 (disposable staging, exact SHA, serialized one-time build, SHA + build-time input + image-ID recording, deterministic image references), WP-P7-07 (staging verification bound to SHA S + build-time inputs + image identities), the teardown window between WP-P7-07 PASS and WP-P7-08 (destroy staging runtime/state, retain verified images, no build/pull), WP-P7-08 (same SHA, same retained images, no rebuild/pull, fail closed on any mismatch), and WP-P7-09 (independent production verification).
- The Release 1 runbook gains the RESOURCE PRECHECK, the explicit Model C procedure sequence with deterministic application image references (`forgemind-backend` / `forgemind-worker` / `forgemind-frontend` plus the Compose-resolved inventory render), and the fail-closed mismatch handling (return to staging verification with a new candidate evidence boundary if the SHA differs, a build-time input value differs, an image identity differs, a verified image is missing, or an operator accidentally rebuilt after staging verification).
