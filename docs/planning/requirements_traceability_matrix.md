...
| FR-09 | Audit — critical reads, agent runs, approvals, writes logged | PARTIALLY IMPLEMENTED — `backend/app/models/audit.py`, `backend/app/services/audit_service.py` (create-only), `backend/app/api/audit.py` (read-only), `backend/app/schemas/audit.py` (WP-REC-04B, incorporated via PR #99); audit emission via WP-REC-04A/04C pending | `tests/unit/test_audit_model.py`, `tests/unit/test_audit_service.py`, `tests/integration/test_api_audit.py`, `tests/integration/test_audit_migration.py` | — | **AT-011**, **AT-012** |
...
| WP-REC-04-DEC (Phase 6 contract and decomposition) | COMPLETE / INCORPORATED | Decision and planning package (DEC-052, accepted 2026-08-15) incorporated into main through PR #97 (regular merge commit `19d41f75cbaedfb652054fc11e5e46562f9581dc`); decomposition `docs/planning/wp_rec_04_decomposition.md`; Phase 6 reconnaissance COMPLETE; AT-009–AT-012 NOT PASS; WP-REC-04B COMPLETE / INCORPORATED (audit-event backend foundation, PR #99); Phase 6 implementation IN PROGRESS; next planned implementation package WP-REC-04A (approval-request backend service), NOT STARTED |
...
- FR-06 now has implementation via WP-REC-03C (COMPLETE). FR-09 is PARTIALLY IMPLEMENTED (WP-REC-04B audit-event backend foundation incorporated via PR #99; audit emission via WP-REC-04A/04C pending). FR-08 and FR-12 reference capabilities not yet implemented — marked as such; no nonexistent file paths cited.
