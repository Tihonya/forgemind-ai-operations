...
| FR-08 | Human Approval — procurement task needs explicit confirmation | PARTIALLY IMPLEMENTED — `backend/app/models/approval.py`, `backend/app/services/approval_service.py`, `backend/app/schemas/approval.py`, `backend/app/api/approval.py` (WP-REC-04A, incorporated via PR #102); procurement-task creation (WP-REC-04C) and Approval Center UI (WP-REC-04D) remain pending | `tests/unit/test_approval_model.py`, `tests/unit/test_approval_service.py`, `tests/integration/test_api_approval.py`, `tests/integration/test_approval_migration.py` | _Not yet implemented_ | **AT-009**, **AT-010** |
| FR-09 | Audit — critical reads, agent runs, approvals, writes logged | PARTIALLY IMPLEMENTED — `backend/app/models/audit.py`, `backend/app/services/audit_service.py` (create-only), `backend/app/api/audit.py` (read-only), `backend/app/schemas/audit.py` (WP-REC-04B, incorporated via PR #99); approval-decision audit emission via WP-REC-04A incorporated via PR #102; procurement audit emission via WP-REC-04C remains pending | `tests/unit/test_audit_model.py`, `tests/unit/test_audit_service.py`, `tests/integration/test_api_audit.py`, `tests/integration/test_audit_migration.py` | — | **AT-011**, **AT-012** |
...
| WP-REC-04-DEC (Phase 6 contract and decomposition) | COMPLETE / INCORPORATED | Decision and planning package (DEC-052, accepted 2026-08-15) incorporated into main through PR #97 (regular merge commit `19d41f75cbaedfb652054fc11e5e46562f9581dc`); decomposition `docs/planning/wp_rec_04_decomposition.md`; Phase 6 reconnaissance COMPLETE; AT-009–AT-012 NOT PASS; WP-REC-04B COMPLETE / INCORPORATED (audit-event backend foundation, PR #99); WP-REC-04A COMPLETE / INCORPORATED (approval-request backend, PR #102); Phase 6 implementation IN PROGRESS; next planned implementation package WP-REC-04C (procurement-task backend), NOT STARTED |
...
- FR-06 now has implementation via WP-REC-03C (COMPLETE). FR-09 is PARTIALLY IMPLEMENTED (WP-REC-04B audit-event backend foundation incorporated via PR #99; approval-decision audit emission via WP-REC-04A incorporated via PR #102; procurement audit emission via WP-REC-04C remains pending). FR-08 is PARTIALLY IMPLEMENTED (approval-request backend via WP-REC-04A, incorporated via PR #102; procurement-task creation via WP-REC-04C and Approval Center UI via WP-REC-04D remain pending). FR-12 references capabilities not yet implemented — marked as such; no nonexistent file paths cited.
