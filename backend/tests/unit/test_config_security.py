...
class TestJWTAlgorithmConfig:
    ...
    def test_jwt_algorithm_rejects_non_hs256(self) -> None:
        with pytest.raises(ValidationError):
            Settings(jwt_algorithm="RS256")  # type: ignore[arg-type]
...
class TestSecretKeyValidation:
    def test_development_accepts_default_secret(self, monkeypatch: MonkeyPatch) -> None:
        monkeypatch.setenv("ENVIRONMENT", "development")
        s = Settings()
        assert len(s.secret_key) >= 32

    def test_production_rejects_default_secret(self, monkeypatch: MonkeyPatch) -> None:
        monkeypatch.setenv("ENVIRONMENT", "production")
        insecure = "dev-secret-key-change-in-production-must-be-32-chars-min"
        with pytest.raises(ValidationError):
            Settings(secret_key=insecure)

    def test_staging_rejects_default_secret(self, monkeypatch: MonkeyPatch) -> None:
        monkeypatch.setenv("ENVIRONMENT", "staging")
        insecure = "dev-secret-key-change-in-production-must-be-32-chars-min"
        with pytest.raises(ValidationError):
            Settings(secret_key=insecure)

    def test_production_accepts_real_secret(self, monkeypatch: MonkeyPatch) -> None:
        monkeypatch.setenv("ENVIRONMENT", "production")
